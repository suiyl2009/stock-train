...
list.req = function(page){
    ...
    var listconf = conf.get('list'); // 显示条目
    var confGlobal = conf.get("global");
    list.m_items = [];
    list.m_items = listconf.show.slice(0);

    gdata.get();
    list.m_totalCodes = gdata.m_data.codes.length;  // 代码列表
    ...
    for (var i = page * list.m_countPage; (i < list.m_totalCodes) && (i < (page + 1) * list.m_countPage) ; i++) {
        codes.push(gdata.m_data.codes[i]);
    }
    console.log('list.m_totalCodes: ' + list.m_totalCodes + ', codes' + codes);

    /*
    // 获取远程显示信息
	//{"code": ["000002", "000004"], "method" : "list.show", "item" : ["Code", "Name", "DayDate", "DayClose", "Revenue"]}
	//{"method": "list.show", "retcode": "0000", "result": [{"Code": "000002", "Name": "万 科Ａ", "DayDate": "20180404",
	// "DayClose": "32.80", "Revenue": "1171.01"}, {"Code": "000004", "Name": "国农科技", "DayDate": "20180404", 
    ...
    */
    ...
    var req = { "code": codes, "method": "list.show", "item": list.m_items, "date": confGlobal.date, "datetype": strDateType };
    function remoteListshow(jsonRes) {
        console.log(jsonRes);
        if ('0000' != jsonRes.retcode) {
            console.log('ERROR: { "method": "list.show" } retcode error: ' + jsonRes.retcode);
            return false;
        }
    ...
    gdata.send(req, remoteListshow);
    console.log('list.m_data: ');
    console.log(list.m_data);

    list.m_currPage = page;
    list.m_selectRow = 0;
    gdata.m_data.idx = list.m_currPage * list.m_countPage + list.m_selectRow;
    gdata.set();
...
list.selectRow = function (ev) {
    ...
    if (ev.clientX <= list.m_cvLeft) {  // 选择的是左侧曲线类型
        ...
        gdata.m_data.idx = list.m_currPage * list.m_countPage + list.m_selectRow;
        gdata.set();
        console.log("gdata.m_data.idx: " + gdata.m_data.idx + ", list.m_selectRow: " + list.m_selectRow);
