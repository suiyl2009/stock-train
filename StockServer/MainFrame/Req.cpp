...
int CReq::Init()          // 初始化接口列表
{
    ...
    m_mapMethod["index.list"]        = &CReq::IndexList;
    m_mapMethod["index.single"]      = &CReq::IndexSingle;
    m_mapMethod["index.batch"]       = &CReq::IndexBatch;
    m_mapMethod["index.kline"]       = &CReq::IndexKLine;
...
{
    int nRet = 0;
    CIndex *ptrIndex = new CIndex;
    nRet = ptrIndex->list(jReq, jRes, strRetCode);
    if (0 != nRet){
        ERROR("Call ptrIndex->list failed. strRetCode: "<<strRetCode);
    }
    delete ptrIndex;
    ptrIndex = NULL;
    return nRet;
    ...
    )
{
    int nRet = 0;
    CIndex *ptrIndex = new CIndex;
    nRet = ptrIndex->single(jReq, jRes, strRetCode);
    if (0 != nRet){
        ERROR("Call ptrIndex->single failed. strRetCode: "<<strRetCode);
    }
    delete ptrIndex;
    ptrIndex = NULL;
    return nRet;
    ...
    )
{
    int nRet = 0;
    CIndex *ptrIndex = new CIndex;
    nRet = ptrIndex->batch(jReq, jRes, strRetCode);
    if (0 != nRet){
        ERROR("Call ptrIndex->batch failed. strRetCode: "<<strRetCode);
    }
    delete ptrIndex;
    ptrIndex = NULL;
    return nRet;
    ...
    )
{
    int nRet = 0;
    CIndex *ptrIndex = new CIndex;
    nRet = ptrIndex->kline(jReq, jRes, strRetCode);
    if (0 != nRet){
        ERROR("Call ptrIndex->kline failed. strRetCode: "<<strRetCode);
    }
    delete ptrIndex;
    ptrIndex = NULL;
    return nRet;
