...
int CSingleLine::GetDayInfo()
{
	...
	for (int i=0; i<vecOneEarn.size(); i++)  // 计算pe及pb
	{
		...
		for (; (j< m_vecDayInfo.size()) && (m_vecDayInfo[j].date>vecOneEarn[i].iDeclarationDate); j++)
		{
			...
			if (m_vecDayInfo[j].pb<-5000) m_vecDayInfo[j].pb = -5000;
			m_vecDayInfo[j].pr = (vecOneEarn[i].iOneEarn != 0) ? m_vecDayInfo[j].close*vecOneEarn[i].iOneAsset/(vecOneEarn[i].iOneEarn*vecOneEarn[i].iOneEarn) : 0;
			if (m_vecDayInfo[j].pr>1000) m_vecDayInfo[j].pr = 1000;
			if (m_vecDayInfo[j].pr<-1000) m_vecDayInfo[j].pr = -1000;
		...
		{
			m_vecDayInfo[t].pr2 = (vecOneEarn[i].iOneEarn != 0) ? m_vecDayInfo[t].close*vecOneEarn[i].iOneAsset/(vecOneEarn[i].iOneEarn*vecOneEarn[i].iOneEarn) : 0;
			if (m_vecDayInfo[t].pr2>1000) m_vecDayInfo[t].pr2 = 1000;
			if (m_vecDayInfo[t].pr2<-1000) m_vecDayInfo[t].pr2 = -1000;
