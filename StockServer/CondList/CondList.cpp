...
CClosingDateCond::CClosingDateCond()
{
    m_ptrStock = NULL;
    m_bReport  = true;
}   

int CClosingDateCond::GetItem(
    int idx,
    int &iValue,
    int iType
    )
{
    string strDate = m_ptrStock->m_vecQuart[m_objListIdx.m_iQuartIdx1]
        .m_rQuart[m_objListIdx.m_iQuartIdx2].m_strClosingDate;
    
    iValue = DateStr2Int(strDate);
    return 0;
}

CDeclareDateCond::CDeclareDateCond()
{
    m_ptrStock = NULL;
    m_bReport  = true;
}
...
{
    string strDate = m_ptrStock->m_vecQuart[m_objListIdx.m_iQuartIdx1]
        .m_rQuart[m_objListIdx.m_iQuartIdx2].m_strDeclarationDate;
	...
	)
{
	...
	for (int i=0; i<m_vecStock.size(); i++)
	{
		...
		if (0 != m_ptrCondValue->Init(m_vecStock[i], m_vecIdx[i]))
		{
	...
	)
{
	...
	for (int i=0; i<m_vecStock.size(); i++)
	{
        ...
        //DEBUG("iStockValue: "<<iStockValue<<", iValue: "<<iValue);
		if ((iStockValue >= iValue) && (true == bGreater)) // 大于等于的情况
		{
		    objResult.m_iDate    = m_vecStock[i]->m_vecQuart[idx].m_rQuart[3].m_iDate;
