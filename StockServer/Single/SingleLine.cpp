...
int CSingleLine::GetDayInfo()
{
	...
	for (int i=0; i<vecOneEarn.size(); i++)  // 计算pe及pb
	{
		...
		for (; (j< m_vecDayInfo.size()) && (m_vecDayInfo[j].date>vecOneEarn[i].iDeclarationDate); j++)
		{
            ...
            m_vecDayInfo[j].rp = (0 == m_vecDayInfo[j].close*vecOneEarn[i].iOneAsset) ? 0 : 
                10000*(vecOneEarn[i].iOneEarn*vecOneEarn[i].iOneEarn)/(m_vecDayInfo[j].close*vecOneEarn[i].iOneAsset);
            if ((0 > vecOneEarn[i].iOneEarn) && (0 < m_vecDayInfo[j].rp)) m_vecDayInfo[j].rp = m_vecDayInfo[j].rp - 2*m_vecDayInfo[j].rp;
            if (m_vecDayInfo[j].rp>1000) m_vecDayInfo[j].rp = 1000;
			if (m_vecDayInfo[j].rp<-200) m_vecDayInfo[j].rp = -200;
	...
	)
{
    ...
    if (0 != m_iDate)
    {
		...
		t = 0;
        //for (int x=0; x<m_vecQuartReport.size(); x++)
        //{
        //    DEBUG("x: "<<x<<", date: "<<m_vecQuartReport[x].m_iDate);
        //}
		while (i <= j) // 二分查找
		{
		    t = (i+j)/2;
			...
			{
			    //DEBUG("Find item. t: "<<t<<", m_iDate: "<<m_vecQuartReport[t].m_iDate<<", iQDate: "<<iQDate);
				break;
			}
			else if (iQDate < m_vecQuartReport[t].m_iDate)
			{
			    //DEBUG("iQDate < m_iDate. t: "<<t<<", date: "<<m_vecQuartReport[t].m_iDate);
			    i = t+1;
			}
			else
			{
		...
		DEBUG("iDate: "<<iDate<<", iQDate: "<<iQDate<<", t: "<<t);
    }
	else
	{
	    t = 0;
	...
	DEBUG("m_idxQBegin: "<<m_idxQBegin<<", date: "<<m_vecQuartReport[m_idxQBegin].m_iDate
	    <<", m_idxQEnd: "<<m_idxQEnd<<", date: "<<m_vecQuartReport[m_idxQEnd].m_iDate
	    <<". m_idxDBegin: "<<m_idxDBegin<<", date: "<<m_vecDayInfo[m_idxDBegin].date
	    <<", m_idxDEnd: "<<m_idxDEnd<<", date: "<<m_vecDayInfo[m_idxDEnd].date);
...
int CSingleLine::GetRPYLine(int &iMax, int &iMin, string &strRetCode)
{
    if ((1 == m_mapDYVal.count("DayRP")) || (m_idxDBegin >= m_vecDayInfo.size()))
	{
	    return 0;
	}

	int iTemp = 0;
    iMax = iMin = m_vecDayInfo[m_idxDBegin].rp;
	CYVal * ptrYVal = new CYVal;
	for (int i=m_idxDBegin; i<=m_idxDEnd; i++)
	{
	    iTemp = m_vecDayInfo[i].rp;
	    ptrYVal->m_vecItems.push_back(iTemp);
		iMax = (iMax>iTemp ? iMax : iTemp);
		iMin = (iMin<iTemp ? iMin : iTemp);
		//DEBUG("DayPR m_vecDayInfo[i].pr: "<<m_vecDayInfo[i].pr<<", iTemp: "<<iTemp
