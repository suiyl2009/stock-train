...
int CTradePoint::EndDate(               // 根据最后的交易点日期过滤
    json_t *jReq,
    json_t *&jRes,
    string &strRetCode
    )
{
    ...
    for (int i=0; i<sJsonSize; i++)
    {
        setCode.insert(json_string_value(json_array_get(jItem, i)));
    }
    
    jItem = json_object_get(jReq, "enddate");
    if (jItem && json_is_integer(jItem)) 
    {
        nDate = json_integer_value(json_object_get(jReq, "enddate"));
    }
    if (0 > nDate)
    {
        strRetCode = CErrJsonType;
    ...
    jItem = json_object_get(jReq, "direct");
    if (jItem && json_is_integer(jItem)) 
    {
        nDirect = json_integer_value(json_object_get(jReq, "direct"));
    }
    if (0 > nDirect)
    {
        strRetCode = CErrJsonType;
        ERROR("GetLines json number value error. nDirect: "<<nDirect<<", strRetCode: "<<strRetCode);
        return 1;
    }
    /////////end: 解析json，得到各字段item//////////////////////
    ...
    do
    {
        ...
        nRet = SearchDBEndDate(ullUserId, setCode, nDate, nDirect, vecCond, strRetCode);
        if (0 != nRet)
        {
            ERROR("Call SearchDBPoint error. llUID: "<<ullUserId<<", code size: "<<setCode.size()
                <<", nDate: "<<nDate<<", nDirect: "<<nDirect<<", strRetCode: "<<strRetCode
                <<", nRet: "<<nRet);
            break;
        }

        json_t *jResult = json_array();  // json
        for (int i=0; i< vecCond.size(); i++)
        {
            json_t *jCondItem = json_object();
            json_object_set_new(jCondItem, "code", json_string(vecCond[i].m_strCode.c_str()));
            json_object_set_new(jCondItem, "date", json_integer(vecCond[i].m_nDate));
            
            json_array_append_new(jResult, jCondItem);
        }
...
int CTradePoint::PrCond(               // 根据pr过滤买点
    json_t *jReq,
    json_t *&jRes,
    string &strRetCode
    )
...
int CTradePoint::SearchDBEndDate(
    ...
    )
{
    ...
    {
        if (0 == nDirect){ // 0: 小于等于
            strSQL = "select code, endDate from tb_trade_point where userID=? "
                "and endDate <= ? order by endDate desc";
        }
        else { // 1：大于等于
            strSQL = "select code, endDate from tb_trade_point where userID=? "
                "and endDate >= ? order by endDate desc";
        ...
        {
            SCondData sItem;
            sItem.m_strCode = resultSet->getString(1).c_str();
            sItem.m_nDate = resultSet->getInt(2);
            if (0 == setCode.count(sItem.m_strCode)){
                continue;
            }
            vecCond.push_back(sItem);
        }
