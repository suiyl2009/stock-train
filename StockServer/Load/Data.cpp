...
int StockData::DecorateReport()
{
	...
	for (int i=0; i<m_vecQuart.size(); i++) // 循环处理每个Report数据
	{
		...
		for (int j=3; j>=0; j--)
		{
            ...
            if ((i>0) && (iOneEarnYear > 0)){
                m_vecQuart[i-1].m_rQuart[j].m_iYearROP = 10000*(m_vecQuart[i-1].m_rQuart[j].m_iYearProfit - iOneEarnYear)/iOneEarnYear;
            }
	...
	)
{
	...
	{
		ERROR("NULL == ptrStockData. strCode: "<<strCode);
		return 0;
	}
