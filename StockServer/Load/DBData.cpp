	...
	)
{
    ...
    CIndexData objBaseInfo;
	vector<CIndexData> vecBaseInfo;
	vecBaseInfo.clear();
    
	MYSQL_RES *result;
	MYSQL_ROW row;
	string strSQL = "SELECT code,name,exchange from tb_index_info where valid=1";
	...
	while (row = mysql_fetch_row(result))
	{
        ...
        objBaseInfo.m_strExchange     = (row[2] ? row[2] : "");
		vecBaseInfo.push_back(objBaseInfo);
	}
	mysql_free_result(result);

	DEBUG("GetInfoData vecBaseInfo.size(): "<<vecBaseInfo.size());
	DEBUG("GetInfoData before mapIndex.size(): "<<mapIndex.size());
	for (int idx=0; idx<vecBaseInfo.size(); idx++)
	{
		string strCode = vecBaseInfo[idx].m_strCode;
		iter = mapIndex.find(strCode);
		if (mapIndex.end() != iter)
		{
			ptrIndexData = iter->second;
		}
		else
		{
			ptrIndexData = new CIndexData;
			ptrIndexData->m_strCode = vecBaseInfo[idx].m_strCode;
			ptrIndexData->m_strName = vecBaseInfo[idx].m_strName;
            ptrIndexData->m_strExchange = vecBaseInfo[idx].m_strExchange;
			mapIndex[vecBaseInfo[idx].m_strCode] = ptrIndexData;
		}
	}
	DEBUG("GetInfoData after mapIndex.size(): "<<mapIndex.size());
